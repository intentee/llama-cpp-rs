...
///
/// ```
/// # use llama_cpp_bindings::llama_supports_mlock;
///
...
pub fn list_llama_ggml_backend_devices() -> Vec<LlamaBackendDevice> {
    ...
    for i in 0..unsafe { llama_cpp_bindings_sys::ggml_backend_dev_count() } {
        ...
        let backend = unsafe { llama_cpp_bindings_sys::ggml_backend_dev_backend_reg(dev) };
        let backend_name = unsafe { llama_cpp_bindings_sys::ggml_backend_reg_name(backend) };
